...
public:
    ...
    /// For the host app's own focus-transfer signal (e.g. DC's lc_focus WLX
    /// command). Confirmed via live logging: the host can send an explicit
    /// deactivate call immediately after a legitimate click-based activation
    /// already handled internally by eventFilter() -- observed consistently
    /// as the cause of a "must click twice" symptom (the host's deactivate
    /// arrived between the activating click and the user's next keypress,
    /// so the keypress saw m_isActive == false). Deactivation requests
    /// arriving within the grace window of a recent internal activation are
    /// treated as spurious host noise and ignored, rather than trusted at
    /// face value like setActive() is.
    void setActiveFromHost(bool active);
...
private:
    ...
    bool m_isActive;
    QElapsedTimer m_lastInternalActivationTimer;
